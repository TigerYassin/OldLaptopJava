...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    public static void rectangle2(double x, double y, double width, double height) {
        if (!(width  >= 0)) throw new IllegalArgumentException("half width must be nonnegative");
        if (!(height >= 0)) throw new IllegalArgumentException("half height must be nonnegative");
        double xs = scaleX(x);
        double ys = scaleY(y);
        double ws = factorX(width);
        double hs = factorY(height);
        if (ws <= 1 && hs <= 1) pixel(x, y);
        else offscreen.draw(new Rectangle2D.Double(xs, ys - hs, ws, hs));
        draw();
    }
